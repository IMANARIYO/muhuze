...
class CreatePaymentRequest(BaseModel):
    ...
    The buyer pays the MUHUZE MoMo number shown at checkout **outside** the
    system (via the operator USSD/app). The buyer's own wallet/line numbers
    are therefore optional — they are kept for our records only, so nothing
    here blocks checkout. The stub gateway immediately returns a
    `request_reference` stored as `provider_ref`; the provider callback
    (`POST /payments/{id}/paid`) clears the money and triggers revenue
    derivation.
    """

    order_id: uuid.UUID = Field(description="The order to pay for")
    momo_phone: str | None = Field(
        default=None, max_length=20, description="Buyer's mobile-money wallet number (optional, for our records)"
    )
    airtel_phone: str | None = Field(
        default=None, max_length=20, description="Airtel/MoMo line the payment was sent from (optional)"
    )
    method: str | None = Field(
        default="mobile_money", max_length=30, description="Payment method"
    )
...
class PaymentResponse(BaseModel):
    ...
    status: str = Field(description="pending | awaiting | paid | failed | refunded | partially_refunded")
